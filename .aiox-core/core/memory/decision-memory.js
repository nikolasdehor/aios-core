...
// Fix 1: Module-level constant instead of per-call allocation
...
class DecisionMemory extends EventEmitter {
  ...
  async save() {
    ...
    // Fix 3: Cap decisions in memory too, not just on disk
    ...
    // Fix 2: Wrap writeFileSync in try/catch — log and return silently
    try {
      fs.writeFileSync(filePath, JSON.stringify(data, null, 2), 'utf-8');
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error('[DecisionMemory] Failed to save decisions: ' + err.message);
    }
  }
  ...
  updateOutcome(decisionId, outcome, notes = '') {
    ...
    // Fix 7: Reject PENDING as an outcome update — it's the initial state, not a final outcome
    if (outcome === Outcome.PENDING) {
      throw new Error('Cannot set outcome to pending — it is the initial state, not a valid outcome update');
    }
